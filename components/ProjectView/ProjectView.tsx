...
const ProjectView = ({ project }: { project: Project }) => {
    return (
        <div className={styles.container}>
            <Link href={'/projects'}>
                <a  className={`${styles.projects} link`}><OlderArrow />Projects</a>
            </Link>
            <div className={styles.top}>
                <h1 className={styles.projectTitle}>{ project.title }</h1>
                <p className={styles.date}>{new Date(project.date).toLocaleDateString()}</p>
            </div>
            <div 
                style={{ 
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.7em' }}>
                ...
                {project.website && (
                    <div>
                        <Tab name='Website'/>
                        <p className={styles.stack}><Link href={project.website}><a target={'_blank'}>{project.website}</a></Link></p>
                    </div>
                )}
                {project.repository && (
                    <div>
                        <Tab name='Repository'/>
                        <p className={styles.stack}><Link href={project.repository}><a target={'_blank'}>{project.repository}</a></Link></p>
                    </div>
                )}                
